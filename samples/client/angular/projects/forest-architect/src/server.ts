...
if (isMainModule(import.meta.url) || process.env['pm_id']) {
    ...
    app.listen(port, (error) => {
        ...
        console.log(`🌳 Forest Architect client listening on http://localhost:${port}`);
        console.log(`🌿 Connecting to Forest Architect agent at http://localhost:10005`);
    });
}
...
async function createOrGetClient() {
    // Create a client pointing to the Forest Architect agent's Agent Card URL (port 10005).
    client ??= await A2AClient.fromCardUrl('http://localhost:10005/.well-known/agent-card.json', {
