...
"""Forest Architect AgentExecutor for A2A integration."""
...
class ForestArchitectAgentExecutor(AgentExecutor):
    """Forest Architect AgentExecutor."""

    def __init__(self, base_url: str):
        # Instantiate two agents: one for UI and one for text-only.
        self.ui_agent = ForestArchitectAgent(base_url=base_url, use_ui=True)
        self.text_agent = ForestArchitectAgent(base_url=base_url, use_ui=False)
    ...
    async def execute(
        ...
        if ui_event_part:
            ...
            if action == "budget_changed":
                new_budget = ctx.get("budget", 500000)
                query = f"RECALCULATE_METRICS: Budget changed to ₹{new_budget:,}"

            elif action == "area_changed":
                new_area = ctx.get("area", 10000)
                query = f"RECALCULATE_METRICS: Area changed to {new_area:,} sq ft"

            elif action == "generate_proposal":
                budget = ctx.get("budget", 500000)
                area = ctx.get("area", 10000)
                org_name = ctx.get("organizationName", "Client Organization")
                query = f"GENERATE_PROPOSAL: For {org_name} with budget ₹{budget:,} and area {area:,} sq ft"

            elif action == "get_recommendations":
                layer = ctx.get("layer", "")
                query = f"GET_SPECIES_RECOMMENDATIONS: For layer {layer}"

            elif action == "design_forest":
                budget = ctx.get("budget", 500000)
                area = ctx.get("area", 10000)
                query = f"DESIGN_FOREST: Budget ₹{budget:,}, Area {area:,} sq ft"
        ...
        async for item in agent.stream(query, task.context_id):
            ...
            if action in ["generate_proposal"]:
